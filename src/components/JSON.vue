...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <p>We operate in:</p>
      <ul>
        <li v-for="location in bookstores.countries" :key="location">
          {{ location }}
        </li>
      </ul>
      <p>Our #1 seller:</p>
      <p>{{ bookstores.topSellers }}</p>
    </section>
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="author in authors" :key="author.id"
          :class="{ 'highlighted-author': author.name === 'George Orwell' }"
          :style="author.name === 'George Orwell' ? highlightedStyle : {}">
          {{ author.name }} ({{ author.birthYear }})
        </li>
      </ul>
    </section>
